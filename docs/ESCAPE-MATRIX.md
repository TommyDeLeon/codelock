...
| D24 | Spend a daily skip, or end the session from another device | `HOLDS` | **Was a lockout until 7 Sep 2026 — the opposite failure.** Pressing Skip resolved the session server-side as `BYPASSED`, and the shell was never told: the screen stayed covered over a session that no longer existed, so no submission could be graded and no unlock token could be minted. The only exit left was D21, which records a failure the user did not earn. A live lock now polls `GET /v1/lock/active` every 5s and releases when the server no longer holds that session (`lock-watchdog.ts`). It fails closed: an unreachable or erroring API is never read as an unlock, so pulling the network is not a bypass. |
