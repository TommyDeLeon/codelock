...
Everything above assumes one machine you control. If you cannot get one — no
card, no Oracle capacity, a locked-down laptop — there is a second path, and it
is entirely free. Read the trade-offs before taking it; they are real.

### Why this is possible

The judge needs a Docker daemon and no managed platform will give you one. But
the API never talks to "the judge" — it talks to a **Judge0-compatible HTTP
API** (`apps/api/src/services/judge0.ts`), which in `docker-compose.yml` happens
to be the local container. Point it at a hosted Judge0 and the one piece that
truly required a VM moves off your infrastructure. The API itself is an ordinary
long-lived Node process.

| Piece | Where | Account |
|---|---|---|
| Web | Vercel | yes |
| API | Render, from `render.yaml` | yes |
| Postgres | Neon | yes |
| Code execution | Hosted Judge0 (RapidAPI) | yes |

### What you give up

- **The speed gate stops being a measurement.** This is the important one. The
  product's whole argument is `best x 1.35 + 40ms`, timed inside a container
  under known load. On a shared public judge, timings vary with whoever else is
  using it and are not comparable to the `bestRuntimeMs` records the gate is
  derived from. The demo still shows the idea; it stops being rigorous, and you
  should not practise against it daily and trust the verdicts.
- **Free instances sleep.** Render spins down after roughly fifteen minutes idle
  and cold-starts in about a minute. The session reaper does not run while it is
  asleep, so abandoned sessions close out late rather than on time.
- **Submission quotas are small.** RapidAPI's Judge0 CE free tier is on the
  order of fifty submissions a day. Fine for a demo, not for practice.
- **Submitted code leaves your infrastructure.** It goes to a third party.

Moving back later is one environment variable: point `JUDGE0_URL` at a
self-hosted judge and everything above is undone.

### Steps

**1. Postgres.** Create a project at [neon.tech](https://neon.tech) and copy the
**pooled** connection string — not the direct one. The API holds a connection
pool and the free tier caps direct connections.

**2. Code execution.** Subscribe to Judge0 CE on RapidAPI (free tier). The URL
is `https://judge0-ce.p.rapidapi.com` and the host header is
`judge0-ce.p.rapidapi.com`.

**3. API.** In Render: **New → Blueprint**, pick this repository. It reads
`render.yaml` from the root and prompts for what it cannot know:
...
API_URL        https://codelock-api.onrender.com
```

The JWT secrets and `ENCRYPTION_KEY` are generated by Render. **Copy
`JWT_UNLOCK_SECRET` out afterwards** — the desktop app needs the identical value
in its `config.json`, and without it the app refuses to start a timer at all
rather than locking you behind a screen it cannot open.

**4. Point the web app at the API.** In the Vercel project set:

```
CODELOCK_API_URL      https://codelock-api.onrender.com
NEXT_PUBLIC_SITE_URL  https://your-app.vercel.app
```

`CODELOCK_API_URL` is read per request and takes effect on redeploy.
`NEXT_PUBLIC_SITE_URL` is compiled into the bundle, so it needs a rebuild — and
until it is set every shared link previews a broken image pointing at
`localhost:3000`.

**5. Check it, in this order.**

```bash
curl https://codelock-api.onrender.com/v1/health
```

Expect `{"ok":true,"database":"up","latencyMs":<n>}`. The first request after a
sleep takes about a minute; that is the free tier, not a fault.

```bash
curl -X POST https://codelock-api.onrender.com/v1/demo/grade -H 'content-type: application/json' -d '{"language":"JAVASCRIPT","sourceCode":"console.log(1)"}'
```

A JSON verdict means execution works end to end. A 502 or a timeout usually
means `JUDGE0_KEY` or `JUDGE0_HOST` is wrong — the API surfaces the upstream
failure rather than pretending the submission was merely incorrect.

Then open the site and run the demo. If the page loads but the demo reports it
cannot reach CodeLock, `CODELOCK_API_URL` is unset or the deployment predates
it; check `https://your-app.vercel.app/runtime-config.js`, which shows the
origin the browser is actually being handed.
