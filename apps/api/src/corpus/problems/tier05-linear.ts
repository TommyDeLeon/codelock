...
const linkedList = {
  ...
  PYTHON: `class LinkedList:
    ...
    def toArray(self):
        ...
        return a`,
  JAVA: `class LinkedList { static class Node{int x;Node next;Node(int x){this.x=x;}} Node head,tail;int n; public void addFirst(int x){Node q=new Node(x);q.next=head;head=q;if(tail==null)tail=q;n++;} public void addLast(int x){Node q=new Node(x);if(tail!=null)tail.next=q;else head=q;tail=q;n++;} public int removeFirst(){if(head==null)return -1;int x=head.x;head=head.next;n--;if(head==null)tail=null;return x;} public int get(int i){if(i<0)return -1;Node q=head;while(q!=null&&i-->0)q=q.next;return q==null?-1:q.x;} public int size(){return n;} public int[] toArray(){int[]a=new int[n];Node q=head;for(int i=0;i<n;i++){a[i]=q.x;q=q.next;}return a;} }`,
  CPP: `class LinkedList { struct Node{int x;Node*next;Node(int v):x(v),next(nullptr){}};Node*head=nullptr,*tail=nullptr;int n=0;public: void addFirst(int x){Node*q=new Node(x);q->next=head;head=q;if(!tail)tail=q;n++;} void addLast(int x){Node*q=new Node(x);if(tail)tail->next=q;else head=q;tail=q;n++;} int removeFirst(){if(!head)return -1;Node*q=head;int x=q->x;head=head->next;delete q;n--;if(!head)tail=nullptr;return x;} int get(int i){if(i<0)return -1;Node*q=head;while(q&&i-->0)q=q->next;return q?q->x:-1;} int size(){return n;} vector<int> toArray(){vector<int>a;for(Node*q=head;q;q=q->next)a.push_back(q->x);return a;} };`,
...
func(l *LinkedList) Get(i int) int{if i<0{return -1};q:=l.head;for q!=nil&&i>0{q=q.next;i--};if q==nil{return -1};return q.x}
